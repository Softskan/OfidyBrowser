...
import com.ofidy.ofidybrowser.bus.BookmarkEvents;
import com.ofidy.ofidybrowser.bus.BrowserEvents;
import com.ofidy.ofidybrowser.constant.BookmarkPage;
import com.ofidy.ofidybrowser.constant.Constants;
import com.ofidy.ofidybrowser.constant.HistoryPage;
import com.ofidy.ofidybrowser.controller.UIController;
import com.ofidy.ofidybrowser.database.BookmarkManager;
import com.ofidy.ofidybrowser.database.HistoryDatabase;
import com.ofidy.ofidybrowser.database.HistoryItem;
import com.ofidy.ofidybrowser.interpolator.BezierDecelerateInterpolator;
import com.ofidy.ofidybrowser.pref.UserPrefs;
import com.ofidy.ofidybrowser.receiver.NetworkReceiver;
import com.ofidy.ofidybrowser.search.SuggestionsAdapter;
import com.ofidy.ofidybrowser.ui.browser.BookmarksView;
import com.ofidy.ofidybrowser.ui.browser.BrowserPresenter;
import com.ofidy.ofidybrowser.ui.browser.BrowserView;
import com.ofidy.ofidybrowser.ui.browser.TabsView;
import com.ofidy.ofidybrowser.ui.dialog.BrowserDialog;
...
import com.ofidy.ofidybrowser.ui.view.AnimatedProgressBar;
import com.ofidy.ofidybrowser.ui.view.Handlers;
import com.ofidy.ofidybrowser.ui.view.LightningView;
import com.ofidy.ofidybrowser.ui.view.SearchView;
import com.ofidy.ofidybrowser.utils.ConfigHelper;
import com.ofidy.ofidybrowser.utils.DrawableUtils;
import com.ofidy.ofidybrowser.utils.ProxyUtils;
import com.ofidy.ofidybrowser.utils.ThemeUtils;
import com.ofidy.ofidybrowser.utils.UrlUtils;
...
public abstract class BrowserActivity extends ThemableBrowserActivity implements BrowserView, UIController, OnClickListener, OnLongClickListener {
    ...
    public void showAddToCartButton(boolean show){
        mAddtoCart.setVisibility(show ? View.VISIBLE : View.GONE);
        //mAddtoCart.
    }
    ...
    private void showQtyDialog(){
        ...
        final AlertDialog alertDialog = new AlertDialog.Builder(this)
                .setTitle("Quantity?")
                .setMessage("Enter the quantity you want")
    ...
    private void showSizeDialog(){
        ...
        v.findViewById(R.id.quantity_down).setOnClickListener(view -> {
            if(size > 1)
                ...
                size = 1;
            inputQty.setText(String.valueOf(size));
        });
        v.findViewById(R.id.quantity_up).setOnClickListener(view -> {
            size += 1;
            inputQty.setText(String.valueOf(size));
        ...
        final AlertDialog alertDialog = new AlertDialog.Builder(this)
                .setTitle("Size?")
                .setMessage("If size applies to your item please choose the size you want. Else click cancel")
                .setPositiveButton("Continue", (dialog, which) -> {
                    String s = inputQty.getText().toString().trim();
                    if(TextUtils.isDigitsOnly(s)) {
...
//        }
    ...
    private final Object mBusEventListener = new Object() {
         ...
         * {@link com.ofidy.ofidybrowser.ui.fragment.BookmarksFragment} and by the
    ...
    class AddToCartTask extends AsyncTask<String, String, String> {
        ...
        protected void onPreExecute(){
            ...
            progress.setOwnerActivity(BrowserActivity.this);
        ...
        protected void onPostExecute(String result){
            ...
            if(TextUtils.isEmpty(result)){
                Snackbar.make(mAddtoCart, "Network error", Snackbar.LENGTH_SHORT).show();
                return;
            }
            try {
                new AlertDialog.Builder(BrowserActivity.this)
                        ...
                        .show();
                //Snackbar.make(mAddtoCart, Html.escapeHtml(result), Snackbar.LENGTH_LONG).show();
            }catch (Exception e){
                Snackbar.make(mAddtoCart, "Internal application error", Snackbar.LENGTH_SHORT).show();
                e.printStackTrace();
            }
        }
    }
